...
public class XmlDetagger extends CasAnnotator_ImplBase {
  ...
  public void process(CAS aCAS) throws AnalysisEngineProcessException {
    ...
    */
    /*
     * getContext().getLogger().log(Level.INFO, "Sofa _InitialView has language " + aCAS.getView("_InitialView").getDocumentLanguage());    
     * getContext().getLogger().log(Level.INFO, "Sofa xmlDocument has language " + aCAS.getView("xmlDocument").getDocumentLanguage());
     * 
     * '_InitialView' is a default sofa that exists in the absence of explicitly defined
     * sofas. The input that comes from the previous stage is the explicitly defined
     * (in the CPE descriptor) 'xmlDocument' sofa. So there isn't an '_InitialView' sofa
     * before this code, which creates it as a side effect, and the default language of
     * this sofa is 'x-unspecified'. This causes all result specification compliant
     * downstream annotators which do not define capabilities for 'x-unspecified' to not
     * produce any annotations.
     * 
     * The intended behavior of assigning the input sofa language to the newly defined
     * sofa 'detaggedDocument' is achieved by taking the language from the sofa 
     * 'xmlDocument' instead.
     */
    plainTextView.setDocumentLanguage(aCAS.getView("xmlDocument").getDocumentLanguage());
    // <--- ivo, 2014-08-19
