 ...
 * 
 */
public class NopCollectionReader extends CollectionReader_ImplBase {
  /**
  ...
  /**
   * Name of optional configuration parameter that gives an explicit
   * name for the default view (otherwise defaulting to "_InitialView"
   */
  public static final String PARAM_DEFAULT_VIEW_NAME = "defaultViewName";
  ...
  private String mDefaultViewName;
  ...
  public void initialize() throws ResourceInitializationException {
    ...
    mCurrentIndex = 0;
    mDefaultViewName = (String) getConfigParameterValue(PARAM_DEFAULT_VIEW_NAME);
  ...
  public void getNext(CAS aCAS) throws IOException, CollectionException {
    ...
    /**
     * ivogeorg, 2014-08-22
     * moving all text ingestion into specialized source format
     * converters
     */
...
//    String text = FileUtils.file2String(file, mEncoding);
    ...
    // put document in an explicitly named view CAS
    try {
        vjcas = jcas.createView(mDefaultViewName);
      } catch (CASException e) {
        throw new CollectionException(e);
      }
...
//    vjcas.setDocumentText(text);

    // set language if it was explicitly specified as a configuration parameter
    if (mLanguage != null) {
      vjcas.setDocumentLanguage(mLanguage);
    }
    

/**     
 * Store location of source document in CAS. Depending on the source,
     converters will apply different parsers and tools to extract the textual
     content. 
     Also, this information is critical
     if CAS Consumers will need to know where the original document contents are located.
     For example, the Semantic Search CAS Indexer writes this information into the
     search index that it creates, which allows applications that use the search index to
     locate the documents that satisfy their semantic queries.
*/    
    SourceDocumentInformation srcDocInfo = new SourceDocumentInformation(vjcas);
//    SourceDocumentInformation srcDocInfo = new SourceDocumentInformation(jcas);
    srcDocInfo.setUri(file.getAbsoluteFile().toURI().toString());
    srcDocInfo.setOffsetInSource(0);
    srcDocInfo.setDocumentSize((int) file.length());
    srcDocInfo.setLastSegment(mCurrentIndex == mFiles.size());
    srcDocInfo.setIsLastDocument(mCurrentIndex == mFiles.size());
